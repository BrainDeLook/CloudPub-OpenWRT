...
Аддон автоматически проверяет наличие новой версии при запуске и затем каждые 24 часа. Кнопка **Проверить обновления** запускает проверку вручную, а кнопка **Обновить аддон** появляется только при наличии нового релиза и устанавливает подходящие пакеты клиента и LuCI. После обновления проверка выполняется автоматически.
...
Сервис при старте применяет токен, точечно синхронизирует UCI-публикации и запускает `clo run` под procd с автоперезапуском. Состояние публикаций хранится в `/etc/cloudpub/publications.state`: добавление или удаление одной публикации не пересоздаёт остальные, поэтому их публичные домены сохраняются.
