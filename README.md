...
| `auth` | `none`, `basic` или `form`; форма доступна для HTTP, HTTPS, 1C и WebDAV (для WebDAV — только через браузер) |
| `acl` | правила доступа в формате `email:роль`; роль `writer` — только для WebDAV |
